...
def parsing_context(connection, source_id, limit=12, offset=0):
    candidates = []
    for row in connection.execute("SELECT draft_json FROM parser_candidate WHERE source_id=? AND status='pending' ORDER BY rowid LIMIT ? OFFSET ?", (source_id, limit, offset)):
        draft = json.loads(row[0])
        candidates.append({"anchor_id": draft["main_anchor_block_id"], "title": draft["prompt"],
                           "category": draft["category_path"], "reference": (draft["reference_text"] or "")[:2500],
