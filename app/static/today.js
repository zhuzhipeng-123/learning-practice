...
function applyScope(ids) {
  ...
  scopeInputs.forEach(input => { input.checked = selected.has(input.dataset.moduleId); });
}
applyScope(savedScope);
function scopes() {
  return scopeInputs.filter(input => input.checked).map(input => input.dataset.moduleId);
}
function updateSummary() {
  const selected = scopes().length;
  const summary = selected ? `已选择 ${selected} / ${scopeInputs.length} 份文档，只从这些文档抽题。` : '尚未选择八股文档。';
  document.querySelector('#quota-summary').textContent = summary;
  document.querySelector('#document-scope-count').textContent = `${selected} / ${scopeInputs.length} 份`;
}
scopeInputs.forEach(input => input.addEventListener('change', () => {
  updateSummary(); showSavedTasks(); feedback.hidden = true;
}));
document.querySelector('#select-all-documents')?.addEventListener('click', () => {
  scopeInputs.forEach(input => { input.checked = true; });
  updateSummary(); showSavedTasks(); feedback.hidden = true;
});
updateSummary();
const choices = [...scopeInputs, document.querySelector('#code-target'), document.querySelector('#theory-target')];
const selectionState = () => JSON.stringify(choices.map(input => input.type === 'checkbox' ? input.checked : input.value));
...
generate.addEventListener('click', () => {
  try {
    ...
    const body = {plan_date:generate.dataset.date,code_target:Number(code.value),theory_target:Number(theory.value),module_quotas:{},theory_scope:scopes(),theory_catalog_version:generate.dataset.catalogVersion,fresh_batch:true,expected_batch:generate.dataset.batchKey || ''};
    if (body.theory_target > 0 && !body.theory_scope.length) throw Error('八股题量大于 0 时至少选择一份文档。');
...
adoptLegacy?.addEventListener('click', () => {
  ...
  const body = {plan_date:generate.dataset.date,code_target:Number(code.value),theory_target:Number(theory.value),
    module_quotas:{},theory_scope:scopes(),theory_catalog_version:generate.dataset.catalogVersion,
